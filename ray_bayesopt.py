...
# Updated to use Bayesian hyperparameter optimization via ray tune
...
DEFAULT_CONFIG_DIR = settings.CONFIG_DIR
RAY_HOME = settings.RAY_TUNE_HOME
OVERWRITE = True
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="samples for bayesopt search"
    )
...
def launch_search(exp_config: Union[List[str], str], name: str, workers: int, gpus_per_worker: float, cpus_per_worker: float, eval_only: bool, samples: int, seed: int) -> None:
    ...
    ray_dir = path.join(RAY_HOME, name)
    # the name of this PBT run (run will be stored at `ray_dir`)
    ...
    def train():
        ...
        search_alg = skopt.SkOptSearch(metric='best_masked_loss', mode='min')
    ...
    best_model_dest = path.join(ray_dir, 'best_model')
